...
void test_validate_my_username()
{
     ...
     */
    FILE *username_txt;
    username_txt = fopen("conf/username.txt", "r");
    if (username_txt == NULL)
    {
        perror("Error opening file");
        exit(EXIT_FAILURE);
    }
    TEST_ASSERT_EQUAL_STRING_MESSAGE(my_username(), malloc_username_from_conf_file(username_txt), "The usernames do not match.");
    fclose(username_txt);
}
